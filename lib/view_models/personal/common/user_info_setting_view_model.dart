...
class UserInfoSettingViewModel extends BaseViewModel {
  ...
  void setBirthday(String value) {
    birthday = value;
    setState(() { // 選擇後才reset, DatePicker onTap內要開啟選擇器 故無法直接callBack click
      _resetData();
    });
  }
  ...
  void _checkNickname() {
    if (nickNameController.text.isNotEmpty) {
      nickNameData = ValidateResultData(
          result: RegularExpressionUtil().checkFormatNickName(nickNameController.text),
  ...
  void onPressSave(BuildContext context) {
    ...
    if (!_checkEmptyController()) {
      setState(() {
        ...
        phoneData =
            ValidateResultData(result: phoneController.text.isNotEmpty);
        birthdayData =
            ValidateResultData(result: birthday.isNotEmpty);
      });
      return;
    ...
    UserInfoAPI(onConnectFail: (message) => onBaseConnectFail(context, message))
        .updatePersonInfo(
        name: nickNameController.text, phoneCountry: phoneCountry,
