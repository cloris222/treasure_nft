...
class TradeReserveInfoNotifier extends StateNotifier<CheckReservationInfo?>
  ...
  ///MARK: 存共用的參數
  Future<void> _saveCommInfo(
      int currentDivision, CheckReservationInfo? state) async {
    ///MARK:先存
    if (state != null) {
      await AppSharedPreferences.setJson(
          "tradeReserveInfo_common_$currentDivision", state.toJson());
    }
    ...
    if (info != null) {
      ...
      ///MARK:再存
      await AppSharedPreferences.setJson(
          "tradeReserveInfo_common_$currentDivision", info.toJson());
    }
  }

  ///MARK: 讀取共用的參數
  Future<CheckReservationInfo?> _getCommInfo(int currentDivision) async {
    var json = await AppSharedPreferences.getJson(
        "tradeReserveInfo_common_$currentDivision");
    if (json != null) {
  ...
  String setKey() {
    return format(
        'tradeReserveInfo_user_{reserveDate}_{currentState}_{currentDivision}',
        {
