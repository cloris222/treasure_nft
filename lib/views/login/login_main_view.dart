...
class _LoginMainViewState extends State<LoginMainView> {
  ...
  Widget _buildTop() {
    return Stack(
      children: [Image.asset(AppImagePath.loginBg)],
    );
  }
  ...
  Widget _buildBottom() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      LoginTextWidget(
          hintText: tr('placeholder-account\''), controller: accountController),
      ErrorTextWidget(data: viewModel.accountData),
      Text(tr('password'),
          style: TextStyle(
              fontWeight: FontWeight.bold, fontSize: UIDefine.fontSize14)),
      LoginTextWidget(
          hintText: tr('placeholder-password'), controller: passwordController),
