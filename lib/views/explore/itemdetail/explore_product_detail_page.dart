...
class ExploreItemDetailPage extends ConsumerStatefulWidget {
  const ExploreItemDetailPage({super.key, required this.itemId});

  final String itemId;

  @override
  ConsumerState createState() => _ExploreItemDetailPageState();
}
...
class _ExploreItemDetailPageState extends ConsumerState<ExploreItemDetailPage> {
  ExploreItemDetailViewModel viewModel = ExploreItemDetailViewModel();
  ExploreItemResponseData data =
      ExploreItemResponseData(priceHistory: [], sellTimeList: []);
  ...
  Widget chartView = const ExploreChartView(
    priceHistoryList: [],
    todayPrice: '',
  );
  late Timer timer;
  ...
  void initState() {
    super.initState();
    Future<ExploreItemResponseData> resList =
        viewModel.getExploreItemDetail(widget.itemId);
    resList.then((value) => _setData(value));

    Future<ExploreLevelInfoResponseData> data =
        viewModel.getCheckLevelInfoAPI();
  ...
  _setData(value) {
    ...
    chartView = ExploreChartView(
        priceHistoryList: priceHistoryList, todayPrice: data.price);
    setState(() {});
  }

  _timer(int secondLeft) {
    CountDownTimerUtil().init(
        callBackListener: MyCallBackListener(myCallBack: (sTimeLeft) {
          setState(() {
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Positioned(
                    ...
                    child: GestureDetector(
                        ...
                        child: Image.asset(
                            'assets/icon/btn/btn_arrow_03_left.png',
                            scale: 0.7)))
              ],
            ),
            ...
            /// 提醒標語+倒數計時
            Visibility(
                visible: _checkCountry(userInfo),
                child: Padding(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildContractAddressView(
                      tr('contractAddress'),
                      _buildShortContractAddress(
                          '0xd7bF69a92DD126752FD8Aab5bC07439c56B2Abf5')),
                  _oneRowForm(tr('owner'), _setShowingForm(data.ownerName),
                      false, false),
                  _oneRowForm(
                      tr('price'), _setShowingForm(data.price), true, true),
                  _oneRowForm(tr('resaleIncome'),
                      _setShowingForm(data.growAmount), true, true),
                  _oneRowForm(
                      tr('network'), _setShowingForm('Polygon'), false, false),
                  // test 這是啥 resp內沒有
            ...
            Container(
              ...
              child: Text(
                tr('historicalVal'),
                style: AppTextStyle.getBaseStyle(
                    fontSize: UIDefine.fontSize24, fontWeight: FontWeight.w700),
              ),
            ),
  ...
  Widget _oneRowForm(
      String title, List<String> contentList, bool bIcon, bool bContentColor) {
    return Padding(
        padding: const EdgeInsets.fromLTRB(0, 0, 0, 6),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.start,
          ...
          children: [
            ...
            bIcon
                ...
                : Container(),
            const SizedBox(width: 4),
            Column(children: _getFormContent(contentList, bContentColor)),
          ],
        ));
  }
  ...
  List<Widget> _getFormContent(List<String> contentList, bool bContentColor) {
    ...
    for (int i = 0; i < contentList.length; i++) {
      textView.add(Text(
        contentList[i],
        style: AppTextStyle.getBaseStyle(
            color: bContentColor ? AppColors.textBlack : AppColors.dialogGrey,
            fontSize: UIDefine.fontSize14,
            fontWeight: FontWeight.w500),
      ));
    }
  ...
  void _pressReserve() {
    Future<dynamic> result =
        viewModel.getNewReservationAPI('ITEM', widget.itemId, (errorMessage) {
      _getErrorResponse(errorMessage);
    });
  ...
  void _checkResponse(dynamic value) {
    if (value == 'SUCCESS') {
      ...
      // 預約等級不符
      ExploreReserveInsertResponseErrorData errorData =
          ExploreReserveInsertResponseErrorData();
      errorData = value;
      _showResultDialog(
          DialogImageType.warning,
          tr('reserve-failed-level'),
          format(tr("reserve-APP_0041'"),
              {'level': errorData.level, 'needLevel': errorData.needLevel}),
          tr('goLevelUp'), () {
  ...
  void _getErrorResponse(String errorMessage) {
    switch (errorMessage) {
      ...
      case 'APP_0064': // 預約金不足
        ...
        break;
      case 'APP_0013': // 餘額不足
        _showResultDialog(DialogImageType.fail, tr("APP_0013"), '',
            tr('confirm'), () => Navigator.pop(context));
        break;
    }
  }

  void _showResultDialog(DialogImageType type, String title, String content,
  ...
  bool _checkCountry(UserInfoData userInfo) {
    for (int i = 0; i < sellTimeList.length; i++) {
      if (sellTimeList[i].country == userInfo.country) {
        DateTime dateTime;
        ...
        if (secondLeft > 0) {
          // 大於0才是尚未開賣，如是開賣中,另外判是否已過結束時間,但現在沒看到開賣的圖..
          _timer(secondLeft);
          return true;
        }
      }
